 ...
 * APIエラーレスポンス専用DTO
 *
 * 成功時のレスポンスDTOとエラー時のレスポンスを明確に分離し、
 * REST APIの設計原則に従った統一的なエラーレスポンス構造を提供します。
 *
 * 【設計原則】
 * - 成功時とエラー時のレスポンス構造を完全に分離
 * - 国際化対応：messageには翻訳キーを格納
 * - セキュリティ考慮：詳細なエラー情報は含めない
 * - 標準化：一貫したエラーレスポンス形式
 */
@Data
@NoArgsConstructor
...
public class ApiErrorResponseDto {
   ...
   * 国際化対応のエラーメッセージキー
   *
   * フロントエンドのi18nシステムで翻訳される
   * 例: "validation.date.checkInRequired"
   */
