...
public class PriceCalculator {
  ...
  /**
   * 部屋の定員に基づいて価格を計算します（後方互換性のため） 【ダイナミックプライシングの動作】
   * このメソッドは現在日（LocalDate.now()）を使用するため、実行日によって価格が変動します。
   * これは実際のホテル業界の価格戦略を模擬したもので、以下の要素により価格が動的に決定されます： - 需要変動: 日付により需要係数が20日周期で変動（季節性・イベント等をシミュレート） -
   * 時間的変動: 同じ検索条件でも日が変わると価格が変わる（現実のホテル予約と同様） - 予測不可能性: ユーザーが予約タイミングを最適化する行動をシミュレート 【価格固定が必要な場合】
   ...
   */
  /**
   * 複雑なダイナミックプライシング計算アルゴリズム 【アルゴリズム概要】 複数の変動要素を組み合わせた価格計算を行い、リアルタイムで適正価格を算出します。
   * ホテル業界で一般的な「定員割引」「ホテルグレード」「需要変動」を数式化した実装。 【計算ステップ】 1. 引数バリデーション（null安全性の確保） 2.
   * ホテル係数の計算（ホテルIDベースの一貫した価格体系） 3. 定員係数の適用（宿泊人数に基づく効率性割引） 4. 基本価格の算出（基準価格 × 定員係数 × 定員数 × ホテル係数） 5.
   * 需要係数の計算（日付ベースの季節変動） 6. 最終価格の決定（基本価格 × 需要係数、最低価格保証付き） 【数学的背景】 - ホテル係数: ホテルID %
   * 定数による疑似ランダムだが一貫した値 - 需要係数: dayOfYear % 周期による正弦波様の変動パターン - 最低価格保証: Math.max()による下限設定でビジネス要件を満足
   * 【パフォーマンス特性】 - 時間計算量: O(1) - 全て定数時間操作 - 空間計算量: O(1) - 固定メモリ使用量 - 再現性: 同じ引数に対して常に同じ結果を返す（副作用なし）
   */
