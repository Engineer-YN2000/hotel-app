...
export const useI18nValidation = () => {
  ...
  // エラーを設定
  const setError = useCallback((field, messageKey, params = {}) => {
    setErrors(prev => ({
      ...prev,
      [field]: t(messageKey, params) // MessageSource.getMessage(key, args, locale)と同等
    }));
  }, [t]);
  ...
  const getMessage = useCallback((key, params = {}) => {
    return t(key, params);
  }, [t]);
